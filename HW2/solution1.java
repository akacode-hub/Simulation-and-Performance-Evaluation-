...
class solution1 {
    ...
    try 
    {
      ...
      sum.initSumParas();
      double maxzdelay=0;                        /* max delay time       */
      int noofdelay=0;                         /* number of delay job  */
      double time=400.0;                        /* time                 */
      int servernode=0;                    /*number of jobs in the service node at a specified time */
      while ( (line = in.readLine()) != null ) 
      {
        ...
        if (arrival < departure)
          ...
          delay    = 0.0;                       /* no delay          */
        // Calaulate max delay and number of delay
        if(maxzdelay<delay)
        	maxzdelay=delay;
        if(delay!=0.0)
        	noofdelay++;
        ...
        sum.wait    += wait;
        sum.service += service;
        //the number of jobs present in the  the service node at a specified time
      ...
      // homework related anaswers 
      System.out.println("a) maximum delay.............=  "+  f.format(maxzdelay));
      System.out.println("b) the number of jobs in the service node at a specified time ="+  servernode);
      System.out.println("c) the proportion of jobs delayed ="+ f.format((double) noofdelay/index));
      System.out.println(" server utilization ="+ f.format(index/departure* sum.service/index));
